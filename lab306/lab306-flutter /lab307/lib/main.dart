...
class _IoTDeviceDashboardState extends State<IoTDeviceDashboard> {
  ...
  Future<List<Telemetry>> fetchTelemetry(int deviceId) async {
    try {
      ...
      print("Lỗi fetch telemetry: $e");
    }
    return [];
  }

  Future<void> _showTelemetryDialog(int deviceId, String name) async {
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            const SizedBox(height: 20),
            const Text("🎮 Nhập lệnh điều khiển",
                style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),

            TextField(
              controller: _payloadController,
              decoration:
                  const InputDecoration(hintText: "{data:20} hoặc {action:on}"),
            ),
          ],
        ),
      ),
...
class Telemetry {
  ...
  factory Telemetry.fromJson(Map<String, dynamic> json) {
    return Telemetry(
      ...
      value: json["payload"] ?? json["value"] ?? "N/A",
    );
  }
}